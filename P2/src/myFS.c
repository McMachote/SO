...
int myMkfs(MiSistemaDeFicheros* miSistemaDeFicheros, int tamDisco,
	...
	miSistemaDeFicheros->fdDiscoVirtual = open(nombreArchivo, O_CREAT | O_RDWR,
	S_IRUSR | S_IWUSR);
	...
	printf("1 bloque para MAPA DE BITS, que cubre %u bloques, %u B\n",
			...
			(unsigned int) (NUM_BITS * TAM_BLOQUE_BYTES));
	...
	MAX_BLOQUES_CON_NODOSI, (unsigned int) sizeof(EstructuraNodoI),
			(unsigned int) MAX_NODOSI);
...
int escribeMapaDeBits(MiSistemaDeFicheros* miSistemaDeFicheros) {
	if (lseek(miSistemaDeFicheros->fdDiscoVirtual,
	TAM_BLOQUE_BYTES * MAPA_BITS_IDX, SEEK_SET) == (off_t) -1) {
...
int escribeNodoI(MiSistemaDeFicheros* miSistemaDeFicheros, int numNodoI,
	...
	if (lseek(miSistemaDeFicheros->fdDiscoVirtual, posNodoI, SEEK_SET)
			== (off_t) -1) {
		perror("Falló lseek en escribeNodoI");
		return -1;
	}
	if (write(miSistemaDeFicheros->fdDiscoVirtual, nodoI,
			sizeof(EstructuraNodoI)) == -1) {
...
int escribeSuperBloque(MiSistemaDeFicheros* miSistemaDeFicheros) {
	if (lseek(miSistemaDeFicheros->fdDiscoVirtual,
	TAM_BLOQUE_BYTES * SUPERBLOQUE_IDX, SEEK_SET) == (off_t) -1) {
...
int escribeDirectorio(MiSistemaDeFicheros* miSistemaDeFicheros) {
	if (lseek(miSistemaDeFicheros->fdDiscoVirtual,
	TAM_BLOQUE_BYTES * DIRECTORIO_IDX, SEEK_SET) == (off_t) -1) {
...
int leeMapaDebits(MiSistemaDeFicheros* miSistemaDeFicheros) {
	...
	if (read(miSistemaDeFicheros->fdDiscoVirtual,
			miSistemaDeFicheros->mapaDeBits, sizeof(BIT) * NUM_BITS) == -1) {
...
int leeNodosI(MiSistemaDeFicheros* miSistemaDeFicheros) {
	...
	if (lseek(miSistemaDeFicheros->fdDiscoVirtual, posNodoI, SEEK_SET)
			== (off_t) -1) {
		perror("Falló lseek en leeNodoI");
		return -1;
	}
	for (i = 0; i < MAX_NODOSI; i++) {
		//nodoI = miSistemaDeFicheros->nodosI[i];
		if (read(miSistemaDeFicheros->fdDiscoVirtual, &nodoI,
				sizeof(EstructuraNodoI)) == -1) {
			perror("Falló read en leeNodoI");
		}
	}
	sync();
	return 0;
...
int myMount(MiSistemaDeFicheros* miSistemaDeFicheros, char* nombreArchivo) {
	...
	if ((miSistemaDeFicheros->fdDiscoVirtual = open(nombreArchivo, O_RDWR))
			== -1) {
	...
	if (leeSuperbloque(miSistemaDeFicheros) != 0) {	//////va por aqui
	...
	printf("SF: %s, %d B (%d B/bloque), %d bloques\n", nombreArchivo,
			miSistemaDeFicheros->superBloque.tamDiscoEnBloques
					* TAM_BLOQUE_BYTES, TAM_BLOQUE_BYTES,
			miSistemaDeFicheros->superBloque.tamDiscoEnBloques);
	printf("1 bloque para SUPERBLOQUE (%u B)\n",
			(unsigned int) sizeof(EstructuraSuperBloque));
	printf("1 bloque para MAPA DE BITS, que cubre %u bloques, %u B\n",
