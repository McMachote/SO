...
static int my_unlink (const char *path){
	int idFilePosition = 0, idxNodoI = 0, i, idBlq;
	if( (idFilePosition = buscaPosDirectorio(&miSistemaDeFicheros, (char*)path+1)) == -1){//No existe el fichero
		return -ENOENT;
	}
	else{
		idxNodoI = miSistemaDeFicheros.directorio.archivos[idFilePosition].idxNodoI;
		EstructuraNodoI *nodorl = miSistemaDeFicheros.nodosI[idxNodoI];

		for(i = 0; i < nodorl->numBloques; i++){
			idBlq = nodorl->idxBloques[i];
			miSistemaDeFicheros.mapaDeBits[idBlq] = 0;
		}
		//Recalcular tamannos
		miSistemaDeFicheros.nodosI[idxNodoI]->libre = 1;
		miSistemaDeFicheros.directorio.archivos[idFilePosition].libre = 1;
...
static int my_rename (const char *oldfile, const char *newfile){
	int idFilePosition = 0;//, idNewFilePosition = 0;
	if( (idFilePosition = buscaPosDirectorio(&miSistemaDeFicheros, (char*)oldfile+1)) == -1){//No existe el fichero
		return -ENOENT;
	}
	if( buscaPosDirectorio(&miSistemaDeFicheros, (char*)newfile+1) != -1){//Existe el fichero newfile => lo borramos
		my_unlink(newfile);
	}
	//punteros sin reservar memoria => no hace falta free??
	EstructuraArchivo* arch = &miSistemaDeFicheros.directorio.archivos[idFilePosition];
	strcpy(arch->nombreArchivo, (char*)newfile+1);
	escribeDirectorio(&miSistemaDeFicheros);
...
struct fuse_operations myFS_operations = {
	...
	.unlink		= my_unlink,					//Eliminar fichero
	//Labo
